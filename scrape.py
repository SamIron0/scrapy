...
def extract_tags(html_content, tags: list[str]):
    ...
    soup = BeautifulSoup(html_content, "html.parser")
    text_parts = []
    classes = ["mntl-recipe-details__item"]
...
async def ascrape_playwright(
    url, tags: list[str] = ["h1", "h2", "h3", "span", "table", "ol"]
) -> str:
    """
...
if __name__ == "__main__":
    url = "https://www.allrecipes.com/recipe/51997/maple-garlic-marinated-pork-tenderloin/"

    async def scrape_playwright():
        results = await ascrape_playwright(url)
        res = await extract(content=results, retries=3)
        return res
